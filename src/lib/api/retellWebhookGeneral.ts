...
export async function processRetellGeneralWebhook(req: NextRequest) {
  try {
    console.log("[retell-webhook] Webhook received");

    // Verify Webhook Token
    // Note: User must configure "x-api-secret" header in Retell Dashboard
    ...
    switch (event) {
      case "call_started":
        ...
        await handleCallStarted(call, supabase);
        break;

      case "call_ended":
        console.log("[retell-webhook] Call ended");
        await handleCallEnded(call, supabase);
        break;

      case "call_analyzed":
        console.log("[retell-webhook] Call analyzed");
        await handleCallAnalyzed(call, supabase);
        break;
...
async function handleCallStarted(call: any, supabase: any) {
...
async function handleCallEnded(call: any, supabase: any) {
...
async function handleCallAnalyzed(call: any, supabase: any) {

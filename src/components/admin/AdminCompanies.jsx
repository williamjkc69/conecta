...
const AdminCompanies = () => {
  ...
  const fetchCompanies = useCallback(async (search, page) => {
    setLoading(true);
    try {
      const { users: fetchedUsers, count } = await fetchUsersWithStats({ role: 'company', searchTerm: search, page, limit });
  ...
  const handleDeleteUser = async (userId, userEmail) => {
    ...
    if (error) {
      toast({ title: "Error", description: `No se pudo eliminar a la empresa ${userEmail}.`, variant: "destructive" });
    } else {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-bold text-slate-100 flex items-center gap-3"><Building /> Gestión de Empresas</h2>
                       ...
                       <AlertDialog>
                          ...
                          <AlertDialogContent className="bg-slate-900 border-slate-700 text-slate-200">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>Esta acción es irreversible. Se eliminará permanentemente la empresa <strong>{user.company_name}</strong> y todos sus datos asociados (vacantes, aplicaciones, etc.).</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>

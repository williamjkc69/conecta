...
const InviteCandidateModal = ({ isOpen, onClose, onSuccess, candidate: initialCandidate, job: initialJob }) => {
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [jobs, setJobs] = useState([]);
  const [selectedJobId, setSelectedJobId] = useState(initialJob?.id || '');
  const [emailSearch, setEmailSearch] = useState('');
  const [foundCandidate, setFoundCandidate] = useState(initialCandidate || null);
  ...
  const fetchJobs = async () => {
    try {
      const { data, error } = await supabase
        .from('jobs')
        .select('id, title, company_id, companies:profiles!jobs_company_id_fkey(company_name)')
        .eq('status', 'active');
  ...
  const handleInvite = async () => {
    ...
    setLoading(true);
    console.log(`[InviteCandidateModal] Inviting ${foundCandidate.email} to job ${selectedJob.title}`);
    ...
    try {
      ...
      toast({
        ...
        description: "El candidato ha sido invitado exitosamente. El estado es ahora 'Invitado'.",
      });
      ...
      toast({
        ...
        description: error.message || "Hubo un problema al procesar la invitación."
      });
    } finally {
      setLoading(false);
    }
      ...
      <DialogContent className="sm:max-w-[500px] bg-slate-900 border-slate-800 text-slate-100">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            {initialCandidate ? (
              <div className="p-3 bg-slate-800 rounded-md border border-slate-700 flex items-center justify-between">
                <div>
                  <p className="font-medium text-slate-200">{initialCandidate.full_name}</p>
                  <p className="text-sm text-slate-400">{initialCandidate.email}</p>
                </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedJobId} onValueChange={setSelectedJobId} disabled={!!initialJob}>
              ...
              <SelectContent className="bg-slate-800 border-slate-700 text-slate-100">
                {jobs.map((job) => (
                  <SelectItem key={job.id} value={job.id}>
                    {job.title} <span className="text-slate-500 text-xs">({job.companies?.company_name})</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>

...
import { X, UserPlus, Briefcase, Loader2, Search, User, Mail } from 'lucide-react';
...
const AssignCandidateModal = ({ isOpen, onClose, onAssignmentSuccess, jobs }) => {
  ...
  const searchCandidate = useCallback(async (docNumber) => {
    ...
    try {
      ...
      if (error || !data) {
        setFoundCandidate(null);
        if (error && error.code !== 'PGRST116') { // Ignore "No rows found" error for feedback
          throw error;
        }
        toast({
          title: "Candidato no encontrado",
          description: "No se encontró ningún candidato con ese número de documento.",
          variant: "destructive"
        });
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!foundCandidate || !jobId || !profile) {
      toast({
        ...
        description: "Debes encontrar un candidato, seleccionar una vacante y estar autenticado.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke('assign-candidate-to-job', {
        body: {
      ...
      toast({
        ...
        description: data.message || `El candidato ha sido asignado a la vacante.`
      });
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <h2 className="text-3xl font-bold bg-gradient-to-r from-purple-400 to-indigo-400 text-transparent bg-clip-text mb-6">Asignar por Documento</h2>

            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2 text-slate-300">Número de Documento del Candidato *</label>
                <div className="relative">
                    ...
                    <input
                        ...
                        required
                    />
                    {isSearching && <Loader2 className="w-4 h-4 absolute right-4 top-1/2 transform -translate-y-1/2 text-slate-400 animate-spin" />}
                </div>
              </div>
              ...
              <AnimatePresence>
                {foundCandidate && (
                  ...
                  >
                    <h3 className="font-semibold text-slate-200 mb-2">Candidato Encontrado</h3>
                    <div className="space-y-2 text-sm">
                      <p className="flex items-center gap-2 text-slate-300"><User className="w-4 h-4 text-purple-400" /> {foundCandidate.full_name}</p>
                      <p className="flex items-center gap-2 text-slate-300"><Mail className="w-4 h-4 text-purple-400" /> {foundCandidate.email}</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div>
                <label className="block text-sm font-medium mb-2 text-slate-300">Seleccionar Vacante *</label>
                 <div className="relative">

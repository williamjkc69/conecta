...
import { Mic, PhoneOff, Loader2, RotateCcw, AlertTriangle, CheckCircle } from 'lucide-react';
        ...
        if (interviewStatus === 'completed' || interviewStatus === 'reviewed') {
            return (
                <div className="text-center p-6 bg-slate-800/50 rounded-lg border border-slate-700">
                    ...
                    <h3 className="text-lg font-semibold text-white">Entrevista Completada</h3>
                    <p className="text-slate-400 text-sm mt-1">
                        Gracias por completar la entrevista. Tus respuestas han sido guardadas.
                    </p>
                </div>
            );
        }
        ...
        return (
            <div className="text-center p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                <p className="text-slate-400">
                    ...
                    <br/>
                    <span className="text-xs text-slate-500">Estado actual: {interviewStatus}</span>
                </p>
            </div>
        );
    }
      ...
      <div className="flex flex-col items-center space-y-4 w-full">
        {isError && (
           ...
           >
              ...
              <span className="text-sm">Ocurrió un error. Por favor intenta de nuevo.</span>
           </motion.div>
        )}
        ...
        >
          {isError ? (
            ...
            <>
                ...
                {interviewStatus === 'in_progress' ? 'Continuar Entrevista' : 'Realizar Entrevista'}
            </>
          )}
        </Button>
      </div>
  ...
  if (isConnecting) {
     return (
        <div className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold text-slate-200">Conectando con Alex...</h3>
            <p className="text-slate-500 text-sm mt-2">Configurando entorno de entrevista seguro</p>
        </div>
     );
  }
    ...
    return (
      ...
      >
        <div className="mb-6 px-4 py-2 bg-green-500/10 border border-green-500/30 rounded-full flex items-center gap-2">
            ...
            <span className="text-green-400 text-sm font-medium">Entrevista en curso</span>
        </div>
